...
async function wrapReturn<T, V>(stix: V, def: () => T, cb: (s: V) => Promise<T>): Promise<T> {
...
export async function get_diff(nodes: StixObject[], edges: StixRelationshipObject[]): Promise<[StixObject, Delta][]> {
  ...
  return wrapReturn(
    { nodes, edges },
    () => [],
    ({ nodes, edges }) => currentDB.getDiff(nodes, edges),
  );
}
