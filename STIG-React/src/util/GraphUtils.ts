...
export function _addVertices(
  sdos: StixObject[],
  data_source: DataSourceType,
  cy: cytoscape.Core,
): [CollectionReturnValue, StixRelationshipObject[], StixRelationshipObject[]] {
  ...
  try {
    ...
    for (const sdo of sdos) {
      ...
      if (cy.getElementById(sdo.id).length > 0) {
        continue;
      }
...
export function setupNodeCtxMenu(
  ...
  if (connected) {
    ...
    const query_in = {
      ...
      select: (ele: cytoscape.CollectionElements) => queryInOut(ele, cy, query_incoming),
    };

    const query_out = {
      content: 'Query Out',
      select: (ele: cytoscape.CollectionElements) => queryInOut(ele, cy, query_outgoing),
    };

    node_menu = cy.cxtmenu({
      ...props,
      commands: [remove, query_in, select_in, select_out, query_out, select_neighbors],
    } as ContextMenu) as unknown as typeof node_menu;
  } else {
    node_menu = cy.cxtmenu({
      ...props,
      commands: [remove, select_in, select_out, select_neighbors],
    } as ContextMenu) as unknown as typeof node_menu;
  }
}
    ...
    commands: [
      ...
      },
      {
        ...
        select: () => {
          ...
          // its raw_data id with "relationship--" on the front
          if (
            selectedSTIXObjectId &&
            !remainingElementIds.includes(selectedSTIXObjectId) &&
...
export function create_bundle(nodes: CollectionReturnValue): STIGBundle {
  ...
  const bundle: STIGBundle = { type: 'bundle', id: bundle_id, objects: [] };
...
export function exportSelected(fileName: string, cy: cytoscape.Core) {
  ...
  const bundle = create_bundle(selected);
  exportGraph(fileName, bundle);
  return bundle;
}

export function exportAll(fileName: string, cy: cytoscape.Core) {
  const allNodes = cy.$(':visible');
  const bundle = create_bundle(allNodes);
  exportGraph(fileName, bundle);
  return bundle;
}
...
export async function queryToGraph(q: string, cyInstance: cytoscape.Core | undefined) {
  ...
  try {
    const queryReturn = await query(q);
...
export async function queryIncoming(elements: cytoscape.CollectionElements, cy: cytoscape.Core) {
  const collection = elements as unknown as CollectionArgument;
  for (const value of collection.toArray()) {
    let data = value.data('raw_data');
    if (typeof data === 'string') {
    ...
    const incoming = await query_incoming(data);
...
export function layoutByTimeframe(cy: cytoscape.Core) {
  ...
  const maxYTimeline = positionObservedDataNodes(
    observedDataNodes,
    relationshipEdges,
    cy,
    animations,
    positionedAttackPatternNodes,
    startX,
    gap,
  );
...
function positionObservedDataNodes(
  ...
  let maxYTimeline = 0;
  observedDataNodes
    .sort((a, b) => new Date(a.data('raw_data')?.last_observed).getTime() - new Date(b.data('raw_data')?.last_observed).getTime())
...
function getConnectedAttackPatternNodes(
  observedNode: cytoscape.NodeSingular,
  relationshipEdges: cytoscape.Collection,
  cy: cytoscape.Core,
) {
