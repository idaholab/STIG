...
const Graph: React.FC = () => {
  ...
  useEffect(() => {
    if (cyInstance) {
      ...
      setupEdgeCtxMenu(cyInstance, setIsPropertyPanelOpen, selectedSTIXObject, setSelectedSTIXObject, setSelectionExists);
  ...
  const handleDrop = async (event: React.DragEvent) => {
    ...
    if (event.dataTransfer.files) {
      ...
      if (files.length > 0) {
        ...
        for await (const {
          alert: { message, type },
          layout,
        } of importGraphToView(cyInstance, event.dataTransfer.files)) {
    ...
    const newNode = handleAddNewCytoscapeNode(label, type, imageUrl, 'GUI');
  ...
  function handleAddNewCytoscapeNode(label: string, nodeType: string, imgUrl: string, dSource: DataSourceType): CytoscapeNode {
    ...
    const cytoscapeNode: CytoscapeNodeData = {
      ...
      created: stixObjectCategory !== 'sco' ? moment().utc().format('YYYY-MM-DDTHH:mm:ss.SSS[Z]') : undefined,
      // SCOs and Marking Definitions do not have the modified property
      modified:
        stixObjectCategory !== 'sco' && nodeType !== 'marking-definition'
  ...
  return <div ref={cyContainerRef} style={{ width: '100%', height: '100%' }} onDrop={handleDrop} onDragOver={handleDragOver} />;
};
