...
const DBOperationsContainer: React.FC = () => {
  ...
  const { selectedSTIXObject, setSelectedSTIXObject, selectionExists, nodesExist } = useStixPropsContext();
  ...
  function submitter(nodes: NodeCollection, edges: EdgeCollection) {
    ...
    (async () => {
      ...
      const message =
        errors === 0
          ? `Submitted ${nodes}/${stix_nodes.length} node(s) and ${edges}/${stix_edges.length} edge(s)`
  ...
  const importFile = async () => {
    const gen: AsyncGenerator<ImportResult> = importGraphToDB(selectedFiles);
