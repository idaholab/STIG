...
const ButtonDBConnect: React.FC<ButtonDBConnectProps> = ({
  ...
  isConnectProcessing,
  setIsConnectProcessing,
  inDBDeleteProcess,
}) => {
  const { connectedDBProfile, setConnectedDBProfile, connectedDBDriver, setConnectedDBDriver } = useContext(
    ConnectedDBContext,
    ...
    <ButtonBasic
      label={
        <span className="flex items-center">
          ...
          {isConnectProcessing ? <span className="loading loading-spinner loading-xs"></span> : null}
          {dbProfile && !dbProfile?.LastDBOperationSuccessful ? (
            <div className="flex items-center ml-1 tooltip tooltip-bottom tooltip-error" data-tip="ERROR: Unable to Connect">
              <Icon path={mdiAlertCircleOutline} size={0.7} className="!dark:text-error text-error-light" />
            </div>
          ) : null}
        </span>
      }
