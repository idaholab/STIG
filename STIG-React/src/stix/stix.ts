...
function stixTypeCheck(prop: SchemaSTIXProperty, v: unknown): boolean {
  ...
  switch (prop.type) {
    ...
    case 'list':
      return v instanceof Array && v.every((e) => stixTypeCheck({ type: prop.listType } as SchemaSTIXProperty, e));
    case 'float':
      return (
        typeof v == 'number' ||
        (v instanceof Number && (prop.min === undefined || +v >= prop.min) && (prop.max === undefined || +v <= prop.max))
      );
    case 'integer':
      return (
        Math.floor(v as number) === v && (prop.min === undefined || v >= prop.min) && (prop.max === undefined || v <= prop.max)
      );
    case 'dictionary':
...
function stixTypeDefaults(prop: SchemaSTIXProperty): unknown {
  ...
  switch (prop.type) {
    case 'string':
      return prop.default !== undefined ? prop.default : undefined;
    case 'enum':
      return prop.enumType !== undefined ? enum_options[prop.enumType][0] : '';
    case 'timestamp': // TODO: make this more precise
    case 'identifier': // TODO: make this more precise
    case 'binary':
      return undefined;
    ...
    case 'integer':
      return prop.min ?? 0;
    case 'open-vocab':
      return prop.openVocabType !== undefined ? open_vocab_options[prop.openVocabType][0] : '';
    case 'dictionary':
    case 'hashes':
    default:
      return undefined;
...
export const createCytoscapeNode = (
  ...
  const newCytoscapeNode: CytoscapeNodeData = addStixPropertiesToNode(node, isNewNode);
  const nodelabel = getNodeLabel(newCytoscapeNode);
...
function addStixPropertiesToNode(node: CytoscapeNodeData, isNewNode: boolean): CytoscapeNodeData {
  const returnedNode: CytoscapeNodeData = { ...node };
  if (isNewNode) {
...
function getAllProps(schemaObject: SchemaSTIXClass) {
  ...
  while (stack.length) {
    ...
    const superClassObject = schema.find((c) => c.name.replace(/-/g, '') === superClass);
    if (superClassObject) {
...
export function checkProps(object: StixObject): boolean {
  const schemaObject = schema.find((c) => {
  ...
  for (const prop of reqProps) {
    ...
    if (!stixTypeCheck(prop, (object as StixObject)[prop.name])) return false;
  }
...
export function isValidStix(object: StixObject): StixObjectCheck {
  const schemaObject = schema.find((c) => {
    return c.name === object.type;
  });
  if (typeof schemaObject !== 'object') {
    return { isValid: false, invalidReason: `passed StixObject is not an object.` };
  }
  ...
  for (const prop of reqProps) {
    // id_ only exists on the database side. Skip this.
    if (prop.name === 'id_') continue;
    if (!stixTypeCheck(prop, (object as StixObject)[prop.name]))
      return {
        isValid: false,
        invalidReason: `${object?.name ? object?.name : 'Unknown'} contains invalid stix for ${prop?.name}.`,
      };
  }

  for (const key of Object.keys(object)) {
    const prop = allowedProps.get(key);
    if (prop && !stixTypeCheck(prop, object[key]))
      return { isValid: false, invalidReason: `${object[key]} contains invalid stix.` };
  }
...
export function setProps(object: StixObject) {
  ...
  for (const prop of reqProps) {
    ...
    if (prop.name === 'id_') continue;
    if (!stixTypeCheck(prop, (object as StixObject)[prop.name])) {
      (object as StixObject)[prop.name] = stixTypeDefaults(prop);
    }
  }
