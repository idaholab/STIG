...
const applyStyleToCytoscape = (cy: cytoscape.Core, selector: string, style: Css.Node) => {
  cy.style().selector(selector).style(style).update();
};
...
export const updateNodeSelectedStyle = (cy: cytoscape.Core) => {
  const selectedStyle: StylesheetStyle | undefined = generateNodeSelectedStyle().find(
    (style) => style.selector === '.stix_node:selected',
  );
