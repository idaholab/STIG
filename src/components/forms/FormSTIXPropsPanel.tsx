...
export default function FormSTIXPropsPanel({
  selectedProperties,
  stixTypeProps,
  showJson,
}: {
  ...
  return showJson ? (
    ...
    <>
      {[idProperty, typeProperty, specProperty, sourceProperty, targetProperty].map((property, i) =>
        property ? (
          <FormElementTextInput
            key={i}
            type="text"
            value={selectedSTIXObject && selectedSTIXObject[property.name] !== undefined ? selectedSTIXObject[property.name] : ''}
            disabled={true}
            onChange={() => {}}
      ...
      {selectedProperties.map((p, i) => (
        <STIXPropertyRenderer key={i} property={p} />
      ))}
    </>
  );
}
