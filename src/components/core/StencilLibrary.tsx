...
const StencilLibrary: React.FC<StencilLibraryProps> = ({ type, onAddNode, searchText, isPanelOpen }) => {
            ...
            <li key={item.id}>
              <button
                ...
                <img
                  src={item.imageUrl}
                  alt={item.alt}
                  className={` w-8 h-8 max-w-8 min-w-8 ${isPanelOpen ? 'ml-4' : 'm-0'} `}
                />
                {isPanelOpen && <span className="ml-4">{item.alt}</span>}
              </button>
            </li>
          ))
          : isPanelOpen && (
            <AlertComponent
              alertText={`Warning: No ${type.toUpperCase()}s match the filter text`}
              alertType="warning"
