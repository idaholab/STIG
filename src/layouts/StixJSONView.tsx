...
const StixJSONView: React.FC<Props> = ({ stixTypeProps, rows }) => {
  ...
  const handleJsonChange = (e: React.ChangeEvent<HTMLTextAreaElement>) => {
    ...
    try {
      ...
      const invalidKeys = Object.keys(parsedJson).filter((jsonKey) => !stixTypePropNames?.includes(jsonKey));
  ...
  }, [selectedSTIXObject, stixTypeProps]);
          ...
          {stixPropNamesValid ? (
            ...
            <>
              <div className="flex gap-1 mt-1">
                ...
                <p>
                  STIX Property Names Invalid. The following property names are not valid for the current STIX object:&nbsp;
                  {invalidPropNames.map((name, i) => `"${name}"${i !== invalidPropNames.length - 1 ? ', ' : ''}`)}
                </p>
              </div>
