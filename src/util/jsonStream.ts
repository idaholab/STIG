      ...
      state: for (; ;) {
        switch (this.state) {
          ...
          case STATE.OPEN_KEY:
            ...
            if (c !== Char.doubleQuote) throw new Error('Malformed object key should start with "');
            this.buffer = '';
          ...
          case STATE.NUMBER_DIGIT:
            for (; ;) {
              if (Char._0 <= c && c <= Char._9) {
                ...
                if (this.exponent) throw new Error('Invalid number has two exponents.');
                this.buffer += 'e';
                this.exponent = true;
              } else if (c === Char.plus || c === Char.minus) {
                if (!this.buffer.endsWith('e')) throw new Error(`Invalid symbol in number: '${String.fromCharCode(c)}'.`);
                this.buffer += String.fromCharCode(c);
              } else if (c === Char.n) {
                if (this.fraction || this.exponent) throw new Error("Invalid symbol in number: 'n'.");
