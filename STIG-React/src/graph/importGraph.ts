...
export async function* importGraphToDB(files: ArrayLike<File>): AsyncGenerator<ImportResult, undefined, undefined> {
  for (const file of Array.from(files)) {
    ...
    const message =
      errors.length === 0
        ? `Imported ${result?.ncommit ?? 0} node(s) and ${result?.rcommit ?? 0} edge(s) from ${file.name} with ${errors.length} error(s).`
...
async function readFileToObjects(
  file: File,
): Promise<{ nodes: StixObject[]; rels: StixRelationshipObject[]; errors: StixObjectCheck[] }> {
  const nodes: StixObject[] = [];
  const rels: StixRelationshipObject[] = [];
  const errors: StixObjectCheck[] = [];

  for await (const obj of streamStixFile(file)) {
    const check = isValidStix(obj);
    if (check.isValid) {
...
async function commitArray(
  nodes: StixObject[],
  rels: StixRelationshipObject[],
): Promise<{ ncommit: number; rcommit: number; errors: number }> {
  let ncommit = 0,
    rcommit = 0,
    errors = 0;
  const batchSize = 250;
